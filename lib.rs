...
mod erc20 {
    ...
    impl Erc20 {
        ...
        /// 读取allowance
        #[ink(message)]
        pub fn allowance_of(&self, who: AccountId) -> Balance {
            let sender = self.env().caller();
            self.allowances.get(&(sender, who)).unwrap_or_default()
        }
    ...
    mod tests {
        ...
        fn constructor_words() {
            ...
            match decoded {
                Event::Transfer(Transfer { from, to, value }) => {
                    assert!(from.is_none(), "mint to error");
                    assert_eq!(to, Some(accounts.alice), "mint to error");
                    assert_eq!(value, 10000, "mint to error");
                }
                _ => panic!("match error")
            }
        }
        ...
        fn transfer_should_works() {
            ...
            let accounts = ink::env::test::default_accounts::<ink::env::DefaultEnvironment>();
            let res = erc20.transfer(accounts.bob, 10);
            assert!(res.is_ok());
            assert_eq!(erc20.balance_of(accounts.alice), 10000 - 10);
        ...
        fn transfer_should_fail() {
            ...
            let res = erc20.transfer(accounts.charlie, 10);
            assert!(res.is_err());
    ...
    mod e2e_tests {
        ...
        async fn e2e_transfer_works(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
            ...
            let contract_acc_id = client.instantiate(
                ...
                0,
                None,
            ).await.expect("instantiate failed")
                .account_id;
            ...
            let balance_of_alice = client.call(
                &ink_e2e::alice(), balance_of_msg, 0, None,
            ).await.expect("instantiate failed");
            // 验证余额
            assert_eq!(balance_of_alice.return_value(), 100 - 2);
